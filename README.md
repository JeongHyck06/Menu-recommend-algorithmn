...
- "치킨", "면"처럼 메뉴 종류를 말하면 그 메뉴군은 상한을 면제하고 가점(`menu_match_weight`, 기본 0.3)을 받는다. "피자 말고"처럼 제외한 메뉴는 걸러낸다.
- 계열(한식/중식/일식/양식/동남아/분식)과 안주 여부는 원본에 없어 메뉴명 키워드 규칙으로 추정해 붙인다. `data/processed/labeling/cuisine_labels.jsonl`(Claude 채팅 라벨링 결과)이 있으면 그 값을 우선 쓴다. "한식 위주로", "중식 말고", "안주"가 선호·제외 조건이 된다.
- "밥"을 단독으로 말하면 밥류 언급과 한식 선호로 해석하고, 쌀밥·잡곡밥 같은 맨밥은 후보에서 뺀다. 날씨는 조건이 아니지만 연상 메뉴(비: 전·칼국수·수제비, 추움: 국·찌개, 더움: 냉면·냉국)에 작은 가점(`context_match_weight`, 기본 0.05)을 준다. 명시한 조건("얼큰한 국물")을 이기지 않도록 언급 메뉴 가점보다 작게 둔다.
