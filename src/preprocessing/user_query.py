...
RULES = (
    # 확정하지 않는 표현
    Rule("이중부정", UNHANDLED,
         r"(?:안\s*[가-힣]+|[가-힣]+지\s*않[은는]|[가-힣]+\s*없는)\s*(?:건|것|거|게|음식|메뉴)?[은는]?\s*(?:싫|별로|안\s*좋)",
         reason="이중 부정은 방향을 확정하지 않음", example="안 매운 건 싫어"),
    Rule("허용표현", UNHANDLED,
         r"(?:[가-힣]+\s*)?[가-힣]+(?:도|어도|아도|여도)\s*(?:괜찮|상관\s*없|돼|되)",
         reason="허용·관용 표현은 필수·선호 조건으로 확정하지 않음", example="매운 것도 괜찮아"),
    Rule("시원한국물", UNHANDLED, r"시원(?:한|하고|하게)(?=\s*(?:국물|국|탕|찌개|해장))",
